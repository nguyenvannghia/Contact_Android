...
public class ZoomableImageView extends ImageView {
    ...
    public interface OnStartRotationListener {
         ...
         */
        boolean onStartRotation(ZoomableImageView view);
    }
     ...
     * @param defStyle
     */
    public ZoomableImageView(final Context context, final AttributeSet attrs, final int defStyle) {
    ...
    protected void onLayout(final boolean changed, final int left, final int top, final int right, final int bottom) {
        ...
        if (getWidth() == 0 || getHeight() == 0 || !isImage()) return;
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        if (!isImage()) return super.onTouchEvent(event);
     ...
     * @param event: custom
     */
    private void startWaiting(final MotionEvent event) {
     ...
     * @param event: custom
     */
    private boolean processDrag(final MotionEvent event) {
...
//				if (DEBUG) Log.v(TAG, String.format("processDrag:dx=%f,dy=%f", dx, dy));
     ...
     * @param event: custom
     */
    private void startCheck(final MotionEvent event) {
     ...
     * @return: custom
     */
    private boolean processZoom(final MotionEvent event) {
     ...
     * @return: custom
     */
    private float calcScale(final MotionEvent event) {
     ...
     */
    private boolean checkTouchMoved(final MotionEvent event) {
     ...
     * @return: custom
     */
    private boolean processRotate(final MotionEvent event) {
    ...
    @SuppressLint("NewApi")
    private void callOnStartRotationListener() {
     ...
     * @return: custom
     */
    private float calcAngle(final MotionEvent event) {
    ...
    private static float dotProduct(final float x0, final float y0, final float x1, final float y1) {
        return x0 * x1 + y0 * y1;
    }

    private static float crossProduct(final float x0, final float y0, final float x1, final float y1) {
        return x0 * y1 - x1 * y0;
    }

    private static float crossProduct(final Vector v1, final Vector v2) {
     ...
     * @return: custom
     */
    private static boolean ptInPoly(final float x, final float y, final float[] poly) {
...
/*		public Vector dec(final Vector other) {
        ...
        public void dec(final float x, final float y) {
            this.x -= x;
            this.y -= y;
        }
    }
    ...
    private static final class LineSegment {
        ...
        public void set(final float x0, final float y0, final float x1, final float y1) {
            p1.set(x0, y0);
            p2.set(x1, y1);
        }
...
/*		@Override
     ...
     */
    private static boolean checkIntersect(final LineSegment seg, final LineSegment[] segs) {
     ...
     * @param value: custom
     */
    private void setMatrixValue(final int index, final float value) {
        updateMatrixCache();
